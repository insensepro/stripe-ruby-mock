  ...
  module RequestHandlers
    module SubscriptionItems
      ...
      def create_subscription_items(route, method_url, params, headers)
        ...
        require_param(:subscription) unless params[:subscription]
        require_param(:plan) unless params[:plan] || params[:price]

        subscriptions_items[params[:id]] = Data.mock_subscription_item(params.merge(plan: plans[params[:plan]], price: prices[params[:price]]))
      end
      ...
      def update_subscription_item(route, method_url, params, headers)
        ...
        subscription_item.merge!(params.merge(plan: plans[params[:plan]], price: prices[params[:price]]))
      end
    end
  end
end
